...
def home(request):
    ...
    if request.method == "POST":
        ...
        api_request = requests.get(f"https://www.airnowapi.org/aq/observation/zipCode/current/?format=application/json&zipCode={zipcode}&distance=25&API_KEY={os.getenv('API_KEY')}")
        try:
        ...
        api_request = requests.get(f"https://www.airnowapi.org/aq/forecast/zipCode/?format=application/json&zipCode=89129&distance=5&API_KEY={os.getenv('API_KEY')}")
        try:
            api = json.loads(api_request.content)
        except Exception as e:
            api = "Error..."
